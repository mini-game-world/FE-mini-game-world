...
export default class ChatBox {
  ...
  setupKeyboard() {
    ...
    this.player.scene.input.keyboard.on('keydown', (event) => {
      if (event.key === 'Enter') {
        this.toggleChatBox();
    ...
    this.player.scene.input.keyboard.removeCapture(Phaser.Input.Keyboard.KeyCodes.SPACE);
    this.player.scene.input.keyboard.removeCapture(Phaser.Input.Keyboard.KeyCodes.ENTER);
  }
  ...
  sendMessage() {
    ...
    if (message && this.player) {
      ...
      this.player.chatBalloon.showChatMessage(message); // Show message in chat balloon
      this.chatInput.value = '';
    } else {
      this.chatInput.value = '';
    }
