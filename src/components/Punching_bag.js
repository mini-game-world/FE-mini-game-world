import Phaser from "phaser";

class Punching_bag extends Phaser.GameObjects.Image {
  constructor(scene, player) {
    super(scene, player.x, player.y - 100, "punching_bag"); // 'arrow' is the texture key for the arrow image
    this.scene = scene;
    this.player = player;

    this.scene.add.existing(this);
    this.setOrigin(0.5, 1);
    this.setDepth(32);
    this.setScale(0.5);

    // Update the position of the arrow to follow the player
    this.scene.events.on("update", this.updatePosition, this);
  }

  updatePosition() {
    this.setPosition(this.player.x, this.player.y - 100);
  }

  destroy() {
    this.scene.events.off("update", this.updatePosition, this);
    super.destroy();
  }
}

export default Punching_bag;
